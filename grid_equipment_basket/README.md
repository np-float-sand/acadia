...
**RESULT: FAILED — not adopted.** Live run 2026-09-05 on the full 14/15-utility panel (NEE excluded: 0 in-window revisions + FPL on record that its plan "was not driven by data centers"). The timing bar (spec §5) fails every cell: the two testable whole-panel revision-flow series (`all_usd`, `all_pct`) show a *negative* primary-window rank-IC at all three horizons (−0.07 to −0.27, best |t| = 1.54), and the four data-center-attributed series are untestable because every stated/imputed DC-$ vintage is 2026-dated so the z-score never warms. On the exposure-overlay gate (spec §6) the **two-sided scaler fails G1** across its whole 6-combo plateau (−0.056 Sharpe / −0.550 Calmar vs layer-1) — though it only ever leaned *in* (multiplier range [1.00, 1.50]), never out. The **one-directional de-risk leg was never exercised**: the composite z never drops below +0.212 anywhere in the span, so the multiplier is a constant 1.0 in both windows across all 9 combos (`active_days = 0`) and its metrics are just the vol-target-only baseline — reported as n/a, not failed-on-merit. Attempt #15 against this theme; first test of the *commitment*-data class and it lands like the prior forecast/flow negatives. Nothing wired live. Full write-up, tables and honesty caveats: `docs/capex-guidance-signal-results.md`.
