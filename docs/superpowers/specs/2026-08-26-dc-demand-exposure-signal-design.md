...
**If the merchant-path integration does land, do not expect it to validate Layer 3 on the standard 2018-2025 backtest — the wiring is necessary but not sufficient.** Two independent limits, verified against the actual seed data (`grid_resilience/data/seed/hyperscaler_deals.csv`) and `config.py`'s `BACKTEST_START="2018-01-01"` / `BACKTEST_END="2025-12-31"`:

1. **Layer 3's real data only starts in 2024**, so most of the 8-year backtest window would still show a flat, uninformative signal for these tickers regardless of the fix. CEG's first disclosed deal is 2024-09-20 (flat zero for 83% of the window); TLN's first is 2024-03-04 (flat zero for 77%); VST's only deal is 2026-01-09 — entirely after `BACKTEST_END`, so VST is a constant zero for the *entire* backtest even with the integration built.
2. **Even restricted to 2024-2025, the covered cross-section is only 2 names (CEG, TLN).** A cross-sectional z-score needs multiple names per date to carry magnitude — with 2 members, `_zscore_columns` collapses to ±1 (sign only, verified elsewhere in this branch's review). That's too thin for a meaningful IC no matter how correct the wiring is.

The appropriate test, once the integration exists, is a short-window (2024-2025), event-study-style read on CEG/TLN specifically — not a full historical backtest rerun — consistent with this spec's own "short sample is acceptable" framing. Even that read would be low-power (5 disclosed events across 2 tickers) and should be treated as directional, not confirmatory. The two changes that would meaningfully improve the odds of a real signal here are (a) finding more disclosed hyperscaler deals for other tickers already in the universe, widening the cross-section past 2 names, and (b) evaluating on the recent window the data actually covers rather than the full historical backtest.
