...
- **Fix Layer 3's integration before spending any more effort on Layer 3's data.** Under `--arch hard-switch` the merchant sleeve never reads the DC-demand signal at all; the seed table could be perfect or empty and CEG/TLN/VST's factor scores would be identical. This needs a merchant-path integration in `build_factor()` (out of scope for the review fix wave). `revenue_mix`/`dual_track` are a partial workaround for CEG (20%) and TLN (10%) only — VST gets 0% weight in both, because `pass_through=1.00`.
