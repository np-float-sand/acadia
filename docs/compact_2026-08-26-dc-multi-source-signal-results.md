...
- **If the merchant-path integration gets built, temper expectations before rerunning the full backtest — the wiring fix is necessary but very unlikely to be sufficient.** Two limits are independent of the bug and would still apply after it's fixed:
  1. **The backtest window (2018-01-01 to 2025-12-31) mostly predates Layer 3's real data.** CEG's first disclosed deal is 2024-09-20 — flat zero for 83% of the window. TLN's first is 2024-03-04 — flat zero for 77%. VST's *only* deal is 2026-01-09 — entirely after `BACKTEST_END`, so VST stays a constant zero for the whole backtest even with the wiring fixed. Rerunning the full 8-year backtest with Layer 3 wired in would mostly retest a flat, uninformative signal and likely produce another underwhelming IC for a different reason than today's — not because the integration failed, but because it can't manufacture pre-2024 data that doesn't exist.
  2. **Even in the one window with real variation (2024-2025), the covered cross-section is 2 names (CEG, TLN — VST doesn't count in this window).** A cross-sectional z-score needs multiple names per date to carry magnitude; the final review already demonstrated this concretely elsewhere in this doc — with only 2 members, `_zscore_columns` collapses to ±1 (sign only). Two names is too thin for a meaningful IC regardless of wiring correctness.
  - **The right test, if this is pursued, is a short-window (2024-2025), event-study-style check on CEG/TLN specifically** — not a full historical backtest — consistent with the "short sample is acceptable" bar this plan was scoped against from the start. Even that would be low-power (5 events, 2 tickers) and should be read as directional, not a validated result. The two changes that would move this further are (a) finding more disclosed hyperscaler deals for other universe tickers (widening the 2-name cross-section) and (b) accepting any read here will be a short, recent-period read, not a long backtest.
