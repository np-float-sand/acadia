...
**Caveat — ICR baseline has almost no historical coverage.** The cached ICR frame (`grid_resilience/data/cache/icr_*.parquet`) has a 2024-12-31 row that is entirely NaN and a 2025-03-31 row covering only 5 of 23 tickers; real usable coverage (a majority of tickers populated) doesn't start until **2025-06-30** (yfinance's ~4-12 quarter depth limit, already noted generally in CLAUDE.md). For roughly the first 85%+ of the 2018-2025 backtest window, the "Hard switch + ICR" configuration has NO regulated-path signal at all and silently falls back to `build_factor()`'s flat cross-sectional-mean fill for every regulated ticker on every date before mid-2025. The table below is therefore not an apples-to-apples full-history comparison — it mostly measures the last two quarters of 2025 for the ICR row, versus full-history behavior for the DC-queue row (the DC-queue signal, now correctly zone-mapped — see below — has real PJM zonal-load coverage back to 2018).
...
- **FE (FirstEnergy, ATSI+JCPL zones) is now the single highest-scoring name in the universe (+1.842)** — mechanically correct (real queue MW over real zone_size, no mapping bug), but the claim stops there; this is *not* confirmed to be data-center-driven. Decomposed: the score is all level, zero momentum (no new PJM filings in the trailing 90 days at this rebalance). Of the 13,932 MW numerator, 10,412 MW sits in the JCPL zone — 18 projects averaging 578 MW (max 2,083 MW), which reads as large utility-scale generation buildout at least as plausibly as co-located data-center demand, the proxy assumption the whole signal rests on. Separately, `TICKER_NODE_MAP` only gives FE the `ATSI`/`JCPL` zones — FirstEnergy also operates Met-Ed, Penelec, and Potomac Edison territory in PJM, so this score reflects roughly half of FE's actual footprint, not the whole company. Real, new, mechanically sound — but treat as "queue activity in two of FE's zones," not "FE is a data-center winner," until someone checks what those 18 JCPL projects actually are.
